...
export default function PaymentAllocationsModal({ invoiceId, onClose, onChanged }) {
  ...
  async function handleVoid(allocation) {
    const reason = window.prompt(
      tr("دلیل ابطال این ردیف پرداخت را بنویسید:", "اكتب سبب إبطال بند الدفع هذا:", "Bu ödeme satırını iptal etme nedenini yazın:", "Enter the reason for voiding this payment line:")
    );
    if (!reason || !reason.trim()) return;
    try {
      const result = await voidInvoicePaymentAllocation(allocation.id, reason.trim());
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl rounded-3xl bg-[var(--erp-bg-soft)] border border-[var(--erp-border)] p-6 text-[var(--erp-text)] max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-black flex items-center gap-2 text-lg" style={{ color: "var(--erp-accent)" }}>
