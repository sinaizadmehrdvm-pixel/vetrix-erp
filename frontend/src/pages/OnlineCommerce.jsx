      ...
      {tab === "campaigns" && (
        <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-5">
          <form onSubmit={createCampaign} className="erp-surface rounded-3xl p-5 space-y-3">
            ...
            <label className="block text-sm font-bold">{fa ? "شبکه" : "Channel"}<select style={inputStyle} className="w-full rounded-xl p-3 mt-1" value={campaign.channel} onChange={(e) => setCampaign({ ...campaign, channel: e.target.value })}>{channels.map((item) => <option key={item} value={item}>{item === "website" ? (fa ? "وبسایت" : "Website") : item[0].toUpperCase() + item.slice(1)}</option>)}</select></label>
