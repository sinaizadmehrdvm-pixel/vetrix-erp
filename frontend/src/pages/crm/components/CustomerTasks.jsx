  ...
  return (
    <section className="rounded-[2rem] bg-[var(--erp-panel)] border border-[var(--erp-border)] p-5 text-[var(--erp-text)]">
      <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
        <div>
          <h2 className="text-2xl font-black text-[var(--erp-accent)] flex items-center gap-2"><BellRing />{fa ? "وظایف و پیگیری‌های مشتری" : "Customer Tasks & Follow-ups"}</h2>
          <p className="text-[var(--erp-muted)] text-sm mt-2">{fa ? "تماس، جلسه، تحویل کالا، پیگیری وصول مطالبات و یادآوری‌های اختصاصی مشتری" : "Calls, meetings, deliveries, receivable follow-ups and customer reminders"}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_190px_190px] gap-3 mb-4">
        <div className="relative"><Search size={18} className="absolute top-3.5 right-4 text-[var(--erp-muted)]" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={fa ? "جستجو در وظایف..." : "Search tasks..."} className="w-full bg-[var(--erp-panel-solid)] text-[var(--erp-text)] rounded-2xl pr-11 pl-4 py-3 outline-none border border-[var(--erp-border)]" /></div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full bg-[var(--erp-panel-solid)] text-[var(--erp-text)] rounded-2xl px-4 py-3 outline-none border border-[var(--erp-border)]"><option value="all">{fa ? "همه وضعیت‌ها" : "All statuses"}</option><option value="active">{fa ? "فعال" : "Active"}</option><option value="open">{fa ? "باز" : "Open"}</option><option value="doing">{fa ? "در حال انجام" : "Doing"}</option><option value="done">{fa ? "انجام شده" : "Done"}</option><option value="cancelled">{fa ? "لغو شده" : "Cancelled"}</option></select>
        <select value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value)} className="w-full bg-[var(--erp-panel-solid)] text-[var(--erp-text)] rounded-2xl px-4 py-3 outline-none border border-[var(--erp-border)]"><option value="all">{fa ? "همه اولویت‌ها" : "All priorities"}</option><option value="urgent">{fa ? "فوری" : "Urgent"}</option><option value="high">{fa ? "زیاد" : "High"}</option><option value="medium">{fa ? "متوسط" : "Medium"}</option><option value="normal">{fa ? "معمولی" : "Normal"}</option><option value="low">{fa ? "کم" : "Low"}</option></select>
      </div>
      ...
      <div className="space-y-3">
        ...
        {filtered.length === 0 && <div className="rounded-3xl bg-[var(--erp-panel-solid)] border border-[var(--erp-border)] p-8 text-center text-[var(--erp-muted)]">{fa ? "وظیفه‌ای برای نمایش وجود ندارد." : "No tasks to show."}</div>}
      </div>
