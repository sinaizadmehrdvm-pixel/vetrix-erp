  ...
  return <div dir={dir} style={{ color: "var(--erp-text)", maxWidth: 1500, margin: "0 auto" }}>
    ...
    <section style={{ ...card, padding: 18 }}>
      ...
      <div style={{ display: "grid", gap: 10 }}>{policies.map(policy => <article key={policy.id} style={{ padding: 14, border: "1px solid var(--erp-border)", borderRadius: 17, background: "var(--erp-bg)" }}>
        ...
        {policy.status === "draft" && <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 11 }}><input placeholder={text.verify} value={noteById[policy.id] || ""} onChange={e => setNoteById({ ...noteById, [policy.id]: e.target.value })} style={{ ...input, flex: "1 1 300px" }} /><button type="button" disabled={saving} onClick={() => activate(policy)} style={{ border: 0, borderRadius: 12, padding: "10px 14px", background: "var(--erp-success-solid)", color: "var(--erp-success-solid-text)", fontWeight: 900 }}>{text.activate}</button></div>}
