...
export default function FinancialPolicy() {
  const { language, dir, country, currency, decimalPlaces, roundingMode, date, n } = useLanguage();
  ...
  const text = useMemo(() => ({
    ...
    effective: fa ? "تاریخ اجرا" : "Effective date",
  ...
  return <div dir={dir} style={{ color: "var(--erp-text)", maxWidth: 1500, margin: "0 auto" }}>
    ...
    <form onSubmit={submit} style={{ ...card, padding: 18, marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 10 }}>
        ...
        <label>{text.country}<input required minLength={2} maxLength={2} value={draft.country_code} onChange={e => setDraft({ ...draft, country_code: e.target.value.toUpperCase() })} style={input} /></label>
    ...
    <section style={{ ...card, padding: 18 }}>
      ...
      <div style={{ display: "grid", gap: 10 }}>{policies.map(policy => <article key={policy.id} style={{ padding: 14, border: "1px solid var(--erp-border)", borderRadius: 17, background: "var(--erp-bg)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><strong style={{ color: "var(--erp-accent)" }}>{policy.version}</strong><span>{text.status}: {policy.status}</span><span>{policy.country_code} · {policy.currency_code} · {n(policy.decimal_places)}</span><span>{date(policy.effective_from)}</span></div>
