...
export default function BankReconciliation() {
  ...
  const copy={
    ...
    iban:fa?"شماره شبا":"IBAN",ledgerCode:fa?"کد حساب دفتر کل":"GL account code",
  ...
  return <div dir={dir} style={{color:"var(--erp-text)",maxWidth:1500,margin:"0 auto"}}>
    ...
    {summary&&<section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:10,marginBottom:15}}>{[[copy.statement,summary.statement.amount,"var(--erp-accent)"],[copy.ledger,summary.ledger.amount,"#c4b5fd"],[copy.matched,summary.statement.matched_count,"#86efac"],[copy.unmatched,summary.statement.unmatched_count,"#fbbf24"],[copy.difference,summary.difference,summary.reconciled?"#86efac":"#fda4af"]].map(([label,value,color],i)=><div key={label} style={{...card,padding:15}}><div style={{color:"var(--erp-muted)"}}>{label}</div><strong style={{display:"block",fontSize:20,color,marginTop:6}}>{i===2||i===3?n(value):money(value)}</strong></div>)}</section>}
    <section style={{...card,overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:850}}><thead><tr>{[copy.date,copy.description,copy.reference,copy.amount,copy.matched,""].map(x=><th key={x} style={{padding:12,textAlign:"start",color:"var(--erp-accent)"}}>{x}</th>)}</tr></thead><tbody>{!lines.length&&<tr><td colSpan={6} style={{padding:24,textAlign:"center",color:"var(--erp-muted)"}}>{copy.noData}</td></tr>}{lines.map(line=><Fragment key={line.id}><tr>{[date(line.transaction_date),line.description||"—",line.reference||"—"].map((v,i)=><td key={i} style={{padding:12,borderTop:"1px solid var(--erp-border)"}}>{v}</td>)}<td style={{padding:12,color:line.amount>=0?"#86efac":"#fda4af",fontWeight:900}}>{money(line.amount)}</td><td style={{padding:12,color:line.matched?"#86efac":"#fbbf24"}}>{line.matched?<><CheckCircle2 size={16}/> {line.voucher_no}</>:copy.unmatched}</td><td style={{padding:12,whiteSpace:"nowrap"}}>{line.matched?<button onClick={()=>unmatch(line.id)} style={{...button,background:"#78350f",color:"#fef3c7"}}><Unlink size={15}/>{copy.unmatch}</button>:<button onClick={()=>find(line)} style={{...button,background:"var(--erp-panel-solid)",color:"var(--erp-accent)"}}><Link2 size={15}/>{copy.find}</button>} <button onClick={()=>remove(line.id)} style={{...button,background:"#7f1d1d",color:"#fee2e2"}}><Trash2 size={15}/></button></td></tr>{candidates[line.id]?.length>0&&<tr key={line.id+"-matches"}><td colSpan={6} style={{padding:12,background:"var(--erp-glow)"}}>{candidates[line.id].slice(0,8).map(c=><button key={c.voucher_line_id} onClick={()=>match(line.id,c.voucher_line_id)} disabled={!c.exact_amount} style={{...button,margin:4,background:c.exact_amount?"#166534":"var(--erp-panel-solid)",color:c.exact_amount?"white":"var(--erp-muted)",opacity:c.exact_amount?1:.55}}>{copy.voucher} {c.voucher_no} — {date(c.voucher_date)} — {money(c.amount)}</button>)}</td></tr>}</Fragment>)}</tbody></table></section>
  </div>;
}
