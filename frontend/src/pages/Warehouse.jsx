...
export default function Warehouse() {
  ...
  return <div dir={dir} style={{ direction: dir }} className="space-y-6">
    ...
    <div className="bg-[var(--erp-bg-soft)] border border-[var(--erp-border)] rounded-3xl p-5"><div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))" }}>
      <Field label={label.warehouse}><select className={inputClass} value={form.warehouse_id} onChange={e=>{ const id = e.target.value; const selected = warehouses.find(w => String(w.id) === id); setForm({...form, warehouse_id: id, warehouse: selected ? selected.name : defaultWarehouseLabel(language)}); }}><option value="">{defaultWarehouseLabel(language)}</option>{warehouses.filter(w => w.active && !w.is_default).map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select></Field>
      <Field label={label.product}><div className="flex gap-2"><select className={inputClass} value={form.product_id} onChange={e=>setForm({...form, product_id:e.target.value})}><option value="">{label.product}</option>{products.map(p=><option key={p.id} value={p.id}>{p.name} | {language === "fa" ? "موجودی" : language === "ar" ? "المخزون" : language === "tr" ? "Stok" : "Stock"}: {n(p.stock||0)}</option>)}</select><button type="button" onClick={()=>setScannerOpen(true)} className="px-3 rounded-xl bg-[var(--erp-glow)] text-[var(--erp-accent)] flex items-center justify-center" title={language === "fa" ? "اسکن بارکد" : language === "ar" ? "مسح الباركود" : language === "tr" ? "Barkod tara" : "Scan barcode"}><ScanBarcode size={18}/></button></div></Field>
