...
export default function AccountingCore() {
  ...
  async function remove(id) { if (!window.confirm(language === "fa" ? "حساب حذف شود؟" : language === "ar" ? "هل تريد حذف الحساب؟" : language === "tr" ? "Hesap silinsin mi?" : "Delete account?")) return; await deleteAccountingAccount(id); await load(); }
  ...
  return h("div", { style: styles.root },
    h("div", { style: { display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 24 } },
      h("div", null, h("h1", { style: { color: "#22d3ee", fontSize: 38, fontWeight: 900, margin: 0 } }, language === "fa" ? "کدینگ حساب‌ها" : language === "ar" ? "دليل الحسابات" : language === "tr" ? "Hesap Planı" : "Chart of Accounts"), h("p", { style: { color: "#94a3b8" } }, language === "fa" ? "مرحله ۷.۱ هسته حسابداری Vetrix ERP" : language === "ar" ? "المرحلة 7.1 من نواة المحاسبة في Vetrix ERP" : language === "tr" ? "Vetrix ERP Muhasebe Çekirdeği - Aşama 7.1" : "Accounting core phase 7.1")),
      h("div", { style: { display: "flex", gap: 10 } }, h("button", { style: { ...styles.btn, background: "#1e293b", color: "#a5f3fc" }, onClick: async () => { await seedAccountingChart(); await load(); } }, language === "fa" ? "کدینگ پیش‌فرض" : language === "ar" ? "الترميز الافتراضي" : language === "tr" ? "Varsayılan Kodlama" : "Seed"), h("button", { style: { ...styles.btn, background: "#22d3ee", color: "#020617" }, onClick: load }, loading ? "..." : language === "fa" ? "به‌روزرسانی" : language === "ar" ? "تحديث" : language === "tr" ? "Yenile" : "Refresh"))
    ),
