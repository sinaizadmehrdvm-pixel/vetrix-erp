...
import { toJalali, todayJalali, fromJalali } from "../../utils/date";

// A text date field that stores/emits a Gregorian ISO ("YYYY-MM-DD") value
// but displays and accepts Jalali text in fa mode - the app stores every
// business date as ISO, but several pages were showing a Jalali-format
// placeholder ("۱۴۰۵/۰۳/۰۹") while actually reading/writing the raw ISO
...
export default function JalaliDateField({ value, onChange, fa, language, className, placeholder, style }) {
  ...
  const [syncedKey, setSyncedKey] = useState(`${fa}:${value}`);
  const nextKey = `${fa}:${value}`;
  ...
  function setToday() {
    ...
    setText(fa ? todayJalali() : isoToday);
    onChange(isoToday);
  }
