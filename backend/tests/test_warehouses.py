...
class StockBreakdownTests(unittest.TestCase):
    ...
    def _make_product(self, stock=100):
        product = Product(name="Warehouse Test Product", sell_price=1000, stock=stock, company_id=TEST_COMPANY_ID)
        self.db.add(product)
        self.db.commit()
    ...
    def test_all_stock_starts_in_default_warehouse(self):
        ...
        breakdown = stock_breakdown(self.db, product.id, TEST_COMPANY_ID)
        default = self.db.query(Warehouse).filter(Warehouse.is_default.is_(True), Warehouse.company_id == TEST_COMPANY_ID).one()
    ...
    def test_default_warehouse_derives_from_aggregate_minus_others(self):
        ...
        default = self.db.query(Warehouse).filter(Warehouse.is_default.is_(True), Warehouse.company_id == TEST_COMPANY_ID).one()
        branch = Warehouse(name="Branch A", is_default=False, active=True, company_id=TEST_COMPANY_ID)
        self.db.add(branch)
        self.db.commit()
        self.db.refresh(branch)

        apply_warehouse_delta(self.db, branch.id, product.id, 30, TEST_COMPANY_ID)
        self.db.commit()
    ...
    def test_moving_quantity_between_buckets_never_touches_aggregate(self):
        ...
        default = self.db.query(Warehouse).filter(Warehouse.is_default.is_(True), Warehouse.company_id == TEST_COMPANY_ID).one()
        branch = Warehouse(name="Branch B", is_default=False, active=True, company_id=TEST_COMPANY_ID)
        self.db.add(branch)
        self.db.commit()
        self.db.refresh(branch)

        apply_warehouse_delta(self.db, default.id, product.id, -15, TEST_COMPANY_ID)  # no-op: default is derived
        apply_warehouse_delta(self.db, branch.id, product.id, 15, TEST_COMPANY_ID)
        self.db.commit()
